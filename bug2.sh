...
echo "Made it into get_source"
  ...
  do           
    ...
    if [[ $parse ==  'device-aries' ]]; then
	echo "Made it into parse"
      GONK=$(echo $line | cut -c78-117)
      #echo -e $revision
    fi
  done <'sources'
}
...
Debug "Get_Branch_From_Version"
  ...
  if [[ $1 == *$VERSION_25 ]]; then
    GUESSED_VERSION="2.5"
    KNOWN_VERSION=1
  fi
   if [[ $1 == *$VERSION_25B ]]; then
    GUESSED_VERSION="2.5"
    KNOWN_VERSION=1
  fi
    if [[ $1 == *$VERSION_25C ]]; then
    GUESSED_VERSION="2.5"
    KNOWN_VERSION=1
  fi
    if [[ $1 == *$VERSION_25D ]]; then
    GUESSED_VERSION="2.5"
    KNOWN_VERSION=1
  fi
    if [[ $1 == *$VERSION_25E ]]; then
    GUESSED_VERSION="2.5"
    KNOWN_VERSION=1
  fi
    if [[ $1 == *$VERSION_25F ]]; then
    GUESSED_VERSION="2.5"
    KNOWN_VERSION=1
  fi
...
function Get_Device_Type { #Get the device type from the device and convert it in to what we know it as
  ...
  DEVICE_NEXUS4="AOSP"
  DEVICE_ARIES='Xperia Z3 Compact (B2G)'
  ...
  if [[ $DETECTED_DEVICE == *$DEVICE_ARIES* ]]; then
    GUESSED_DEVICE="Aries"
    KNOWN_DEVICE=1
  fi  
...
function Detect_Aries_Firmware {
adb wait-for-device &&
DETECTED_FIRMWARE=`adb shell getprop ro.bootloader`
...
DEFAULT_FIRMWARE=$DETECTED_FIRMWARE
if [[ $DETECTED_FIRMWARE == *$FLAME_v10G_2* ]]; then
  DEFAULT_FIRMWARE="D5803_23.1.A.1.28_NCB.ftf"
  return 0
fi
...
function Choose_Aries_Firmware { #Ask the user which flame firmware to use, providing a default
  Debug "Choose_Aries_Firmware"
  Detect_Aries_Firmware
    FIRMWARE=$DEFAULT_FIRMWARE
